...
export default function SingleTodo({oneTodo}) {
    ...
    useEffect(() => {
        ...
        function func(item){
            ...
            return item.category == value && item.complete == true;
        }
        ...
        for(let i = 0; i < todosFromStorage.length; i++){
            todosList.push({"task": todosFromStorage[i].task, "description": todosFromStorage[i].description,"category": value, "complete": true, "created": todosFromStorage[i].createdOn, "id":todosFromStorage[i]._id});
